...
const {handler, ScriptAndPathInfoResolver, BadRequest} = require('./php-cgi');
...
describe('php-cgi.js', function ()
{
    describe('ScriptAndPathInfoResolver', function ()
    {
        const scriptResolver = new ScriptAndPathInfoResolver('/docroot', 'index.php');
        ...
        describe('PHP', function ()
        {
            ...
            cases.forEach(({args, expected}) =>
            {
                ...
                {
                    expect(scriptResolver.resolveUri(...args)).to.eql(expected);
                });
            });
        });
        ...
        {
            const staticFiles = ['/wp-content/uploads/2001/01/01/image.jpg'];

            staticFiles.forEach((staticFile) => {
                it('should treat it as path_info?', () => {
                    // noinspection BadExpressionStatementJS
                    expect(scriptResolver.resolveUri(staticFile)).to.eql({
                        PATH_INFO: staticFile,
                        PATH_TRANSLATED: '/docroot' + staticFile,
                        REQUEST_URI: staticFile,
                        SCRIPT_FILENAME: '/docroot/index.php',
                        SCRIPT_NAME: '/index.php'
                    });
                });
        ...
        describe('Bad URLs', function ()
        {
            ...
            badPaths.forEach((badPath) =>
            {
                ...
                {
                    expect(() => scriptResolver.resolveUri(badPath)).to.throw(BadRequest);
                });
            });
